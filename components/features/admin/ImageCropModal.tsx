...
export function ImageCropModal({ title = "Upload photo", onClose, onCropped }: Props) {
  ...
  function handleConfirm() {
    ...
    ctx.restore();
    onCropped(out.toDataURL("image/jpeg", 0.82));
    onClose();
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="relative bg-surface border border-border rounded-[--radius-lg] shadow-2xl w-full max-w-sm flex flex-col gap-5 p-6">
        <button onClick={onClose} className="absolute top-3 right-3 p-1.5 rounded hover:bg-surface-raised text-foreground-muted hover:text-foreground transition-colors">
          <X size={16} />
        </button>
        ...
        {!imageSrc ? (
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-2 w-full">
              <button onClick={() => setZoom((z) => Math.max(MIN_ZOOM, z - 0.2))} className="text-foreground-muted hover:text-foreground"><ZoomOut size={15} /></button>
              <input type="range" min={MIN_ZOOM} max={MAX_ZOOM} step={0.01} value={zoom}
                onChange={(e) => setZoom(parseFloat(e.target.value))} className="flex-1 accent-[var(--brand)]" />
              <button onClick={() => setZoom((z) => Math.min(MAX_ZOOM, z + 0.2))} className="text-foreground-muted hover:text-foreground"><ZoomIn size={15} /></button>
            </div>
        ...
        <div className="flex gap-2 justify-end">
          {imageSrc && (
            <Button variant="ghost" size="sm" onClick={() => { setImageSrc(null); imgEl.current = null; }}>Change photo</Button>
          )}
          <Button variant="outline" size="sm" onClick={onClose}>Cancel</Button>
          {imageSrc && (
            <Button size="sm" onClick={handleConfirm}>Use photo</Button>
          )}
        </div>
      </div>
    </div>
